...
if ( ! function_exists( 'themes_starter_setup_theme' ) ) :
    function themes_starter_setup_theme() {
        ...
        load_theme_textdomain( 'ouaprs-theme', get_template_directory() . '/languages' );
...
function get_page_number() {
    if ( get_query_var( 'paged' ) ) {
        print ' | Strana' . get_query_var( 'paged' );
    }
}
...
if ( ! function_exists( 'themes_starter_content_nav' ) ) :
    ...
    ?>
            <div id="<?php echo $nav_id; ?>" class="d-flex mb-4 justify-content-between">
                <div><?php next_posts_link( '<span aria-hidden="true">&larr;</span> Starší příspěvky' ) ?></div>
                <div><?php previous_posts_link( 'Novější příspěvky <span aria-hidden="true">&rarr;</span>' ); ?></div>
            </div><!-- /.d-flex -->
    <?php
        else :
...
if ( ! function_exists( 'themes_starter_article_posted_on' ) ) :
    ...
    function themes_starter_article_posted_on() {
        printf('<span class="sep">Zveřejnil(a) </span>%1$s<span class="sep"> v </span><a href="%2$s" title="%3$s" rel="bookmark"><time class="entry-date" datetime="%4$s">%5$s</time></a></span>',
            get_the_author(),
...
function themes_starter_password_form() {
    ...
    $output = '<div class="row">';
        ...
        $output .= '<h4 class="col-md-12 alert alert-warning">' . __( 'This content is password protected. To view it please enter your password below.', 'ouaprs-theme' ) . '</h4>';
            $output .= '<div class="col-md-6">';
                $output .= '<div class="input-group">';
                    $output .= '<input type="password" name="post_password" id="' . $label . '" placeholder="Heslo" class="form-control" />';
                    $output .= '<div class="input-group-append"><input type="submit" name="submit" class="btn btn-primary" value="Odeslat" /></div>';
